...
class bs_multislot_dyn_ephyl(gr.top_block):
    ...
    def __init__(self, M=32, N=1, T_bch=200, T_g=50, T_p=1000, T_s=150, bs_slots=range(1), cp_ratio=0.25, debug_log=False, ip_decision_layer_addr='localhost', list_sensor=["A","B"], lora_bw=250e3, lora_cr=4, lora_crc=True, lora_sf=7, port_bs_feedback=5562, port_bs_inst=5561, power_tresh=-30, sample_rate=1e6, sn_1_ip_addr='mnode4', sn_2_ip_addr='mnode5'):
        ...
        self.msg_connect((self.hier_bs_0, 'DLCCH'), (self.zeromq_pub_msg_sink_0_0_0, 'in'))
        self.msg_connect((self.zeromq_sub_msg_source_0_0, 'out'), (self.hier_bs_0, 'inst'))
        self.msg_connect((self.zeromq_sub_msg_source_0_0_0, 'out'), (self.hier_bs_0, 'ULCCH'))
        self.msg_connect((self.zeromq_sub_msg_source_0_0_0_0, 'out'), (self.hier_bs_0, 'ULCCH'))
...
def main(top_block_cls=bs_multislot_dyn_ephyl, options=None):
    if options is None:
