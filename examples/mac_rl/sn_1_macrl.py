...
class sn_1_macrl(gr.top_block):

    def __init__(self, M=32, N=1, control='2', cp_ratio=0.25, T_g=50, T_s=150, T_p=1000, T_bch=200, bs_slots=range(10)):
        gr.top_block.__init__(self, "Sn 1 Macrl")
        ...
        self.connect((self.hier_sensor_0_2, 1), (self.uhd_usrp_sink_0_0, 0))
    ...
    def set_control(self, control):
        self.control = control
        self.hier_sensor_0_2.set_control(self.control)
...
def argument_parser():
    ...
    parser.add_option(
        "", "--control", dest="control", type="string", default='2',
...
def main(top_block_cls=sn_1_macrl, options=None):
    if options is None:
        options, _ = argument_parser().parse_args()

    tb = top_block_cls(control=options.control, T_p=options.T_p)
    tb.start()
    tb.wait()
