...
class upper:
    ...
    def send(self, payload):
        ...
        self.log("Sending new instruction - %s" % (payload))
        self.socket_TX.send(pmt.serialize_str(pmt.to_pmt(payload)))

    def extract(self, feedbackPmt):
        """ 
        Extract all information for a feedback message received 
        Input :
            Received feedback at PMT format (default here)
        Return : 
        ...
        feedback = pmt.to_python(feedbackPmt)

        if self.bs:
            # Current frame number
            frame_nbr = int(pmt.to_python(pmt.dict_ref(msg_pmt, pmt.to_pmt("FRAME"), pmt.PMT_NIL))) 
            # List of [sensor ID, ULCCH message]
            ulcch = pmt.to_python(pmt.dict_ref(msg_pmt, pmt.to_pmt("ULCCH"), pmt.PMT_NIL)) 
